class GrassBlock extends Block {
  ...
  interact() {
    // this = new StoneBlock(this.x, this.y)
    return this;
  }
  renderEdge(ctx) {
    ...
    let left = leftBlock && !leftBlock.connectsWithGrass;
    let right = rightBlock && !rightBlock.connectsWithGrass;
    let up = upBlock && !upBlock.connectsWithGrass;
    let down = downBlock && !downBlock.connectsWithGrass;
  ...
  renderEdgePartial(quadrant, portion, ctx) {
    let image = textureLookup("grassEdge");
    if (!image) return;
